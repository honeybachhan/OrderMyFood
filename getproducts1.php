<?php
	...
	if($action=="showAll"){
		...
		$stmt=$dbcon->prepare('SELECT item_name,price,image FROM menu_detail where action=1');
		$stmt->execute();
		
	}else{
		
		$stmt=$dbcon->prepare('SELECT item_name,price,image FROM menu_detail where hid=:cid and action=1');
			...
			<tr>
				<td>
					<div style="border-radius:3px; border:#cdcdcd solid 1px; padding:10px; ;  margin-left:30px; ">
						<?php echo"<img src='food/".$image."'>"; ?></div>
